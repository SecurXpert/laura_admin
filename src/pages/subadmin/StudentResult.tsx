          ...
          <table className="w-full text-left border-collapse min-w-[1100px]">
            <thead>
              <tr className="border-b border-[#f1f5f9]">
                <th className="py-5 px-6 text-[12px] font-bold text-[#94a3b8] uppercase tracking-widest">STUDENT NAME</th>
                <th className="py-5 px-6 text-[12px] font-bold text-[#94a3b8] uppercase tracking-widest">{activeTab === 'exam' ? 'EXAM NAME' : 'QUIZ NAME'}</th>
                <th className="py-5 px-6 text-[12px] font-bold text-[#94a3b8] uppercase tracking-widest">{activeTab === 'exam' ? 'CODING SCORE' : 'MCQ SCORE'}</th>
                <th className="py-5 px-6 text-[12px] font-bold text-[#94a3b8] uppercase tracking-widest">PERCENTAGE</th>
            ...
            <tbody>
              {((activeTab === 'exam' && loadingExam) || (activeTab === 'quiz' && loadingQuiz)) ? (
                ...
                quizResults.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((result, idx) => {
                  ...
                  const name = result.student_name || result.candidate_name || `User ${result.id || idx}`;
                    ...
                    <tr key={idx} className="border-b border-[#f8fafc] hover:bg-[#f8fafc] transition-colors group">
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <div className="text-[15px] font-bold text-[#0f172a] group-hover:text-[#5f5ce6] transition-colors">{name}</div>
                            <div className="text-[13px] font-medium text-[#94a3b8] mt-0.5">{cid}</div>
                          </div>
                        </div>
                      </td>
                      <td className="py-4 px-6 text-[15px] font-bold text-[#334155]">{result.quiz_name || result.quiz_id || result.id || '-'}</td>
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-3">
                          <div className="w-12 h-1.5 bg-[#f1f5f9] rounded-full overflow-hidden">
                            <div className="h-full bg-[#3b82f6] rounded-full" style={{ width: `${Math.min(100, (scoreDisplay / totalQ) * 100)}%` }}></div>
                      ...
                      <td className="py-4 px-6 text-[14px] font-medium text-[#64748b] flex items-center gap-2">
                        <Calendar className="w-4 h-4 text-[#cbd5e1]" />
                        {formatDate(result.date || result.submitted_at || result.submited_at || result.created_at || '')}
                      </td>
                    </tr>
                  )
                })
