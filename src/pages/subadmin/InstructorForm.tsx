...
const InstructorForm = () => {
  ...
  const validateForm = () => {
    ...
    if (!isEdit) {
      ...
      if (!formData.password.trim() || formData.password.length < 8 || formData.password.length > 30) {
        toast({
          title: "Validation Error",
    ...
    if (formData.bio.length > 250) {
      toast({
        title: "Validation Error",
        description: "Bio maximum 250 characters allowed",
        variant: "destructive",
      });
      return false;
    }
    ...
    if (selectedFile) {
      if (selectedFile.size > 5 * 1024 * 1024) {
        toast({
          title: "Validation Error",
          description: "Profile picture size must be up to 5 MB",
          variant: "destructive",
        });
        return false;
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      navigate(formatNav("/dashboard/instructors"));
    } catch (error: any) {
      toast({
        title: "Error",
  ...
  return (
    <div className="space-y-6 w-full overflow-x-hidden box-border p-4 sm:p-6">
      {/* HEADER */}
      <div className="flex items-start gap-3 mb-6 sm:mb-8 w-full box-border">
        <button
          onClick={() => navigate(formatNav("/dashboard/instructors"))}
