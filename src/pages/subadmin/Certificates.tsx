...
export default function Certificates() {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      ...
      try {
        // We'll try the admin/courses endpoint which was previously in the file
        // Alternatively, it could be trainer/courses if logged in as trainer.
    ...
    const fetchCertificates = async () => {
      ...
      if (!courseId) {
        ...
        try {
          ...
          const promises = (Array.isArray(courses) ? courses : []).map((c) =>
                ...
                <div key={i} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 flex flex-col hover:shadow-md transition-shadow h-full">
                  ...
                  <div className="p-6 flex flex-col gap-4 bg-white flex-1">
                    ...
                    <div className="flex gap-3 mt-4 pt-1 items-center">
                      ...
                      <button className="w-12 h-12 rounded-full border border-gray-100 flex items-center justify-center text-gray-400 hover:bg-gray-50 transition-colors shrink-0 shadow-sm">
          ...
          {certArray.length > 0 && (
            <div className="flex flex-col sm:flex-row items-center justify-between mt-6 px-2 gap-4">
              <div className="text-[13px] font-medium text-[#6B7280]">
                Showing {((currentPage - 1) * CARDS_PER_PAGE) + 1}-{Math.min(currentPage * CARDS_PER_PAGE, certArray.length)} of {certArray.length}
              </div>
