...
const AssignCourseToStudent = () => {
  ...
  useEffect(() => {
    const fetchAssignedStudents = async () => {
      try {
        ...
        toast({ title: "Failed to fetch assigned students", variant: "destructive" });
      } finally {
        setLoading(false);
      }
    };
  ...
  // Group the filtered data by student using a Map to preserve newest-first ordering
  const groupedStudents = (() => {
    const map = new Map<number, { student_id: number; student_name: string; courses: { id: number; name: string }[] }>();
    filteredData.forEach((curr) => {
      if (!map.has(curr.student_id)) {
        map.set(curr.student_id, {
          student_id: curr.student_id,
          student_name: curr.student_name,
          courses: [],
        });
      }
      const item = map.get(curr.student_id)!;
    ...
    <div className="space-y-6 relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex justify-end">
          <Button
            size="sm"
            onClick={() => navigate(formatNav("/dashboard/assign-course-student/add"))}
