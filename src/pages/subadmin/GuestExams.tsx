...
const GuestExams = () => {
  ...
  const [courses, setCourses] = useState<{id: number, title?: string, name?: string, course_name?: string}[]>([]);
  ...
  const fetchExamsAndCourses = async () => {
    try {
      ...
      if (Array.isArray(examsRes.data)) {
        data = examsRes.data;
      } else if (examsRes.data && Array.isArray(examsRes.data.data)) {
        data = examsRes.data.data;
      }
  ...
  const getCourseName = (courseId: number) => {
    ...
    return course?.title || course?.name || course?.course_name || `Course ID: ${courseId}`;
  };
            ...
            <div className="overflow-x-auto w-full [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              <table className="w-full text-left border-collapse whitespace-nowrap min-w-max">
                ...
                <tbody className="divide-y divide-gray-50 text-sm">
                  {loading ? (
                    <tr>
                      <td colSpan={10} className="p-8 text-center text-gray-500">Loading exams...</td>
                    </tr>
                  ) : filteredExams.length === 0 ? (
                    <tr>
                      <td colSpan={10} className="p-8 text-center text-gray-500">No exams found</td>
                    </tr>
                  ) : (
                    paginatedExams.map((exam, index) => {
                      ...
                      return (
                        <tr key={exam.id} className="hover:bg-gray-50/50 transition-colors">
                          <td className="p-4 pl-6 font-bold text-gray-900">{exam.title}</td>
                          ...
                          <td className="p-4">
                            <span className="text-[14px] font-semibold px-2 py-1 rounded-full bg-blue-50 text-blue-600">
                              {exam.category}
                            </span>
                          </td>
                          <td className="p-4 text-gray-600 flex items-center gap-1.5 mt-1"><FiClock className="text-gray-400" /> {exam.duration} min</td>
                          <td className="p-4 text-gray-600 text-[14px]">{formatDate(exam.window_start)}</td>
                          <td className="p-4 text-gray-600 text-[14px]">{formatDate(exam.window_end)}</td>
                          ...
                          <td className="p-4 pr-6">
                            <div className="flex items-center justify-center gap-3">
                              <button onClick={() => navigate(formatNav(`/dashboard/guest-exams/view/${exam.id}`))} className="text-indigo-500 hover:text-indigo-700 transition" title="View"><FiEye className="w-4 h-4" /></button>
