    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 w-full">
      ...
      {loading ? (
        ...
        instructors.map((inst) => {
          const profileImg = inst.profile_picture || (inst as any).profile_photo || (inst as any).avatar || (inst as any).image;
          return (
            <div
              key={inst.id}
              className="bg-[#FFFFFFCC] rounded-[18px] p-4 sm:p-6 border border-[#ECECEC] shadow-[0_4px_20px_rgba(0,0,0,0.04)] hover:shadow-[0_6px_24px_rgba(0,0,0,0.08)] transition-all duration-300 flex flex-col justify-between h-full min-w-0 w-full"
            >
              {/* TOP SECTION */}
              <div className="flex flex-col sm:flex-row items-start gap-3 sm:gap-4 w-full min-w-0">
                ...
                <div className="flex-1 min-w-0 w-full">
                  ...
                  {/* FULL BIO WITHOUT TRUNCATION / CUTTING */}
                  <div className="mt-2.5 sm:mt-3 text-[14px] sm:text-[15px] text-[#4B5563] leading-relaxed break-words whitespace-pre-wrap">
              ...
              <div className="flex gap-3 sm:gap-4 mt-5 pt-4 border-t border-[#F0F0F0] w-full mt-auto">
                ...
                <Button
                  onClick={() =>
                    navigate(formatNav(`/dashboard/instructors/edit/${inst.id}`), {
