...
const Reviews: React.FC = () => {
  ...
  const fetchStaticData = async () => {
    try {
      ...
      else if (data && Array.isArray(data.data)) setStudents(data.data);
      else setStudents([]);
    } catch (err) {
    ...
    try {
      ...
      else if (data && Array.isArray(data.courses)) setCoursesList(data.courses);
      else if (data && Array.isArray(data.data)) setCoursesList(data.data);
      else setCoursesList([]);
    } catch (err) {
    ...
    try {
      ...
      else if (data && Array.isArray(data.instructors)) setInstructors(data.instructors);
      else if (data && Array.isArray(data.data)) setInstructors(data.data);
      else setInstructors([]);
    } catch (err) {
  ...
  const fetchReviews = async () => {
    try {
      ...
      toast({ title: "Failed to fetch trainer reviews", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="min-w-[160px] flex-1 sm:flex-none">
              <ScrollableDropdown
                ...
                options={uniqueCourseIds.map(id => ({ value: String(id), label: getCourseTitle(id) }))}
                placeholder="All Courses"
                buttonClassName="bg-[#F8FAFC] border-[#F1F5F9] font-semibold text-[13px] rounded-[16px] h-[46px]"
              />
            </div>
            ...
            <div className="min-w-[160px] flex-1 sm:flex-none">
              <ScrollableDropdown
                ...
                options={uniqueTrainerIds.map(id => ({ value: String(id), label: getInstructorName(id) }))}
            ...
            <div className="min-w-[140px] flex-1 sm:flex-none">
              <ScrollableDropdown
                ...
                options={[5, 4, 3, 2, 1].map(stars => ({ value: String(stars), label: `${stars} Stars` }))}
