...
import { FiPlus, FiSearch, FiEdit, FiTrash2, FiEye, FiUpload, FiX } from "react-icons/fi";

export type CompilerQuestionType = {
  question_id: number;
  title: string;
  question: string;
  description: string;
...
const CompilerQuestions = () => {
  const [questions, setQuestions] = useState<CompilerQuestionType[]>([]);
  ...
  const fetchQuestions = async () => {
    try {
      setLoading(true);
      const res = await api.get("/compiler-questions/get");
      let data = [];
  ...
  const handleDelete = async (questionId: number) => {
    ...
    try {
      await api.delete(`/compiler-questions/delete?question_id=${questionId}`);
      ...
      <>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full md:w-auto">
            <button
              onClick={() => navigate(formatNav("/dashboard/compiler-questions/add"))}
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6 flex flex-col sm:flex-row gap-4 justify-between items-center">
          <div className="relative w-full sm:w-96">
            ...
            <input
              type="text"
              placeholder="Search by title or question ID..."
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden w-full">
          <div className="overflow-x-auto w-full [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            <table className="w-full text-left border-collapse whitespace-nowrap min-w-max">
              <thead>
                <tr className="bg-gray-50/80 border-b border-gray-100 text-[14px] font-bold text-gray-400 uppercase tracking-wider">
              ...
              <tbody className="divide-y divide-gray-50 text-sm">
                {loading ? (
                  <tr>
                    <td colSpan={8} className="p-8 text-center text-gray-500">Loading compiler questions...</td>
                  </tr>
                ) : filteredQuestions.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="p-8 text-center text-gray-500">No questions found</td>
                  </tr>
                ) : (
                  paginatedQuestions.map((q, index) => (
                    <tr key={q.question_id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="p-4 pl-6 font-bold text-gray-900 whitespace-nowrap">{q.title}</td>
                      <td className="p-4 text-gray-500 truncate max-w-[200px]" title={q.question}>{q.question}</td>
                      <td className="p-4 text-gray-500 truncate max-w-[200px]" title={q.description}>{q.description}</td>
                      <td className="p-4 text-gray-600 font-mono text-[13px]">{q.sample_inputs}</td>
                      <td className="p-4 text-gray-600 font-mono text-[13px]">{q.sample_outputs}</td>
                      ...
                      <td className="p-4 pr-6">
                        <div className="flex items-center justify-center gap-3">
                          <button onClick={() => navigate(formatNav(`/dashboard/compiler-questions/view/${q.question_id}`))} className="text-indigo-500 hover:text-indigo-700 transition" title="View">
                            <FiEye className="w-4 h-4" />
                          </button>
                          <button onClick={() => navigate(formatNav(`/dashboard/compiler-questions/edit/${q.question_id}`))} className="text-orange-500 hover:text-orange-700 transition" title="Edit">
...
export default CompilerQuestions;
