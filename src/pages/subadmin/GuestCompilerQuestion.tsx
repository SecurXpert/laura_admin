...
interface GuestCompilerQuestionProps {
  isModal?: boolean;
  onSuccess?: () => void;
  onCancel?: () => void;
}

const GuestCompilerQuestion: React.FC<GuestCompilerQuestionProps> = ({ isModal, onSuccess, onCancel }) => {
  ...
  const handleSubmit = async () => {
    if (!title || !question || !sampleInputs || !sampleOutputs || !courseId) {
      toast({
        title: "Validation Error",
        description: "Title, question, course, sample inputs and sample outputs are required.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      if (isEditMode) {
        await api.put(`/guest/compiler-questions/update?question_id=${id}`, payload);
        toast({
          title: "Updated",
        ...
        await api.post("/guest/compiler-questions/add", payload);
        toast({
          title: "Created",
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-[14px] text-[#64748B] mt-1 font-medium">
            {isEditMode ? `Editing: ${title}` : "Create a new guest compiler challenge"}
          </p>
        </div>
        <button
          onClick={() => isModal && onCancel ? onCancel() : navigate(formatNav("/dashboard/guest-compiler-questions"))}
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="p-6 space-y-5">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="block text-[11px] font-bold text-gray-400 uppercase tracking-wider">Course</label>
              </div>
              <select
                className="w-full border border-gray-200 bg-white p-3 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-sm transition-all"
                value={courseId}
              ...
              >
                ...
                {courses.map((course) => (
                  <option key={course.id} value={String(course.id)}>
                    {course.title || course.name || course.course_name || `Course ${course.id}`}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-[11px] text-gray-400 font-medium">{sampleInputs.length}/50</span>
                </div>
                <input
                  maxLength={50}
...
export default GuestCompilerQuestion;
