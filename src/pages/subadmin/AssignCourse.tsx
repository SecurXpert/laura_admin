...
const AssignCourse = () => {
  ...
  const fetchAssignedCourses = async () => {
    try {
      setLoading(true);
      const res = await api.get(`/admin/assigned-courses-instructor`);
      const reversedData = [...(res.data || [])].reverse();
      setAssigned(reversedData);
    } catch {
      toast({ title: "Failed to fetch assigned courses", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredData = assigned.filter((item) => {
    ...
    return (
      ...
      String(item.instructor_id).includes(value) ||
      String(item.course_id).includes(value)
    );
  });

  // Group the filtered data by instructor using a Map to preserve newest-first ordering
  const groupedInstructors = (() => {
    const map = new Map<number, { instructor_id: number; instructor_name: string; courses: { id: number; name: string }[] }>();
    filteredData.forEach((curr) => {
      if (!map.has(curr.instructor_id)) {
        map.set(curr.instructor_id, {
          instructor_id: curr.instructor_id,
          instructor_name: curr.instructor_name,
          courses: [],
        });
      }
      const item = map.get(curr.instructor_id)!;
      if (!item.courses.find((c) => c.id === curr.course_id)) {
        item.courses.push({
          id: curr.course_id,
    ...
    <div className="space-y-6 relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex justify-start md:justify-end">
          <Button
            size="sm"
            onClick={() => navigate(formatNav("/dashboard/assign-instructor"))}
      ...
      <div className="mt-6 bg-white border border-gray-100 rounded-[24px] overflow-hidden shadow-[0_2px_15px_-3px_rgba(0,0,0,0.05)] pb-2 mb-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 pb-4">
          ...
          <div className="relative w-full md:w-[320px]">
            ...
            <input
              type="text"
              placeholder="Search instructors,name,course,id."
          ...
          <table className="w-full text-sm border-collapse min-w-[800px]">
            ...
            <thead className="bg-[#F9FAFB80]">
              <tr className="border-t-2 border-b-2 border-gray-100">
                <th className="px-6 py-4 text-left text-[14px] font-bold text-[#6B7280] tracking-wider uppercase w-[30%]">INSTRUCTOR</th>
                ...
                <th className="px-6 py-4 text-left text-[14px] font-bold text-[#6B7280] tracking-wider uppercase w-[15%]">INSTRUCTOR ID</th>
              </tr>
            </thead>
            ...
            <tbody className="divide-y-2 divide-gray-100">
              {loading ? (
                ...
                <>
                  {groupedInstructors.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((item, idx) => (
                    <tr key={idx} className="hover:bg-gray-50/50 transition-colors bg-white">
                      {/* INSTRUCTOR NAME */}
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-4">
                          {/* Avatar */}
                          <div className="flex items-center justify-center w-[46px] h-[46px] rounded-full bg-[#8b5cf6] text-white flex-shrink-0 font-medium text-[16px] shadow-sm">
        ...
        {/* ================= PAGINATION ================= */}
        {(!loading && groupedInstructors.length > 0) && (
          <div className="flex flex-col sm:flex-row items-center justify-start px-6 py-5 border-t border-gray-100 gap-6 w-full bg-white">
            <div className="text-[13px] font-medium text-[#6B7280]">
              Showing {groupedInstructors.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0}-{Math.min(currentPage * itemsPerPage, groupedInstructors.length)} of {groupedInstructors.length}
            </div>
            <div className="flex items-center gap-2">
              ...
              {Array.from({ length: Math.ceil(groupedInstructors.length / itemsPerPage) }).map((_, i) => {
                const pageNumber = i + 1;
                if (
                  pageNumber === 1 ||
                  pageNumber === Math.ceil(groupedInstructors.length / itemsPerPage) ||
              ...
              <button
                onClick={() => setCurrentPage((p) => Math.min(Math.ceil(groupedInstructors.length / itemsPerPage), p + 1))}
                disabled={currentPage === Math.ceil(groupedInstructors.length / itemsPerPage) || groupedInstructors.length === 0}
                className="px-4 py-1.5 rounded-full border border-gray-200 text-[#374151] text-[13px] font-semibold hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed bg-white h-[34px] flex items-center justify-center"
              >
                Next
              </button>
