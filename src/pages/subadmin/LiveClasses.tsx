...
import { RefreshCw, Calendar as CalendarIcon, Video, Search, Calendar, ChevronDown, Copy, Trash2 } from "lucide-react";
import ScheduleClassForm from "@/components/subadmin/ScheduleClassForm";
...
const CountdownTimer = ({ targetDate }: { targetDate: string }) => {
  ...
  return (
    <span className="text-[12px] font-bold text-[#8B5CF6] bg-[#F5F3FF] px-2.5 py-1 rounded-full whitespace-nowrap">
      {timeLeft === "Started" ? "Started" : `Starts in: ${timeLeft}`}
    </span>
  );
};
...
export default function LiveClasses() {
  ...
  const fetchLiveClasses = async () => {
    ...
    try {
      ...
      const sortedClasses = [...classArray].reverse();
                  ...
                  return (
                    <div key={cls.id} className="bg-white rounded-[24px] p-6 shadow-[0_4px_24px_rgba(0,0,0,0.02)] border border-gray-100/60 flex flex-col h-full relative group hover:shadow-[0_8px_30px_rgba(0,0,0,0.04)] transition-all">
                      ...
                      <div className="flex justify-between items-start mb-4 gap-2">
                        <h3 className="text-[19px] font-bold text-gray-900 leading-snug pr-2 line-clamp-2">
                          {cls.title}
                        </h3>
                        {isActive && <CountdownTimer targetDate={cls.scheduled_at} />}
                      </div>
