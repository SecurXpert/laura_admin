...
const CreateExamModal: React.FC<CreateExamModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(`Failed to ${examToEdit ? "update" : "create"} exam:`, error);
      toast({
        title: "Error",
        description: error?.response?.data?.detail?.[0]?.msg || `Failed to ${examToEdit ? "update" : "create"} exam. Please try again.`,
        variant: "destructive",
      });
    } finally {
      setLoading(false);
          ...
          <div className="bg-gray-50 w-full max-w-7xl rounded-2xl shadow-xl overflow-hidden max-h-[95vh] overflow-y-auto relative">
            <CompilerQuestion
              ...
              onSuccess={() => {
                ...
                const token = localStorage.getItem('access_token');
                const headers = { Authorization: token ? `Bearer ${token}` : '' };
                api.get('/compiler-questions/get', { headers }).then(res => {
                  let qData = [];
