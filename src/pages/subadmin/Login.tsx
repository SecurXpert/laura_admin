...
export default function AdminLogin() {
  ...
  const saveAuthAndNavigate = (tokenData: any) => {
    let token = "";
    ...
    if (!token || typeof token !== "string") {
      setError("Failed to retrieve a valid access token.");
      return;
    }
    ...
    const dest = window.location.pathname.includes("subadmin")
      ? "/subadmin/dashboard"
      : "/dashboard";
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append("username", email); // In case backend uses OAuth2PasswordRequestForm
      formData.append("password", password);

      const res = await axios.post(`${API_BASE_URL}/subadmin/login`, formData, {
        headers: {
  ...
  const handleVerifyEnroll = async () => {
    ...
    try {
      ...
      let extractedToken = "";
      if (typeof res.data === "string") extractedToken = res.data;
      else if (res.data && typeof res.data === "object") {
        extractedToken = res.data.access_token || res.data.token || res.data.jwt || "";
      }
      if (extractedToken) {
        setFinalAccessToken(extractedToken);
      }
      ...
      if (res.data.backup_codes) {
        ...
        saveAuthAndNavigate(extractedToken);
      } else {
        setStep("finalMfa");
      }
    } catch (err: any) {
        ...
        {step === "backupCodes" && (
          <div className="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] w-full max-w-md p-8 md:p-10 space-y-6 border border-gray-100 text-center">
            ...
            <Button
              onClick={() => {
                if (finalAccessToken) {
                  saveAuthAndNavigate(finalAccessToken);
                } else {
                  setStep("finalMfa");
                }
              }}
