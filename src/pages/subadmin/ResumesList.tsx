...
const ResumesList = () => {
  ...
  const fetchResumes = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${API_BASE_URL}/resumes/`, axiosConfig());
      setResumes(res.data || []);
    } catch (err) {
  ...
  return (
    <div className="w-full max-w-full box-border space-y-6 sm:space-y-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 w-full box-border mb-6">
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white shadow-md">
            <FileText className="w-8 h-8" />
          </div>
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Resumes</h1>
            <p className="text-md sm:text-md text-[#4B5563] mt-1 font-medium">View all submitted resumes</p>
          </div>
        </div>
        {/* POST method removed entirely as requested */}
      </div>
              ...
              >
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-[#615FFF] to-[#AD46FF] text-white text-xs font-semibold shadow-sm hover:opacity-90 transition-opacity"
                  >
          ...
          {resumes.length > 0 && (
            <div className="flex flex-col sm:flex-row items-center justify-between mt-8 px-2 gap-4">
              <div className="text-[13px] font-medium text-[#6B7280]">
                Showing {((currentPage - 1) * itemsPerPage) + 1}-{Math.min(currentPage * itemsPerPage, resumes.length)} of {resumes.length}
              </div>
