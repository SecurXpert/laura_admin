...
export type StudentCompilerQuestion = {
  question_id: number;
  title: string;
  question: string;
  description: string;
...
const StudentCompilerQuestions = () => {
  const [questions, setQuestions] = useState<StudentCompilerQuestion[]>([]);
  ...
  const newQuestionId = location.state?.newQuestionId;
  ...
  const fetchQuestions = async () => {
    try {
      ...
      if (newQuestionId) {
        try {
          const newQRes = await api.get(`/compiler-questions/get?question_id=${newQuestionId}`);
          let newData = null;
          if (Array.isArray(newQRes.data) && newQRes.data.length > 0) {
            newData = newQRes.data[0];
          } else if (newQRes.data?.data && Array.isArray(newQRes.data.data) && newQRes.data.data.length > 0) {
            newData = newQRes.data.data[0];
          } else if (newQRes.data?.question_id) {
            newData = newQRes.data;
          }

          if (newData) {
            const exists = data.some((q: any) => q.question_id === newData.question_id);
            if (!exists) {
              data.unshift(newData);
            }
          }
        } catch (e) {
          console.error("Failed to explicitly fetch the new question", e);
        }
      }
      ...
      <>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full md:w-auto">
            <button
              onClick={() => navigate("/student-compiler-questions/add")}
                ...
                {loading ? (
                  ...
                  paginatedQuestions.map((q, index) => (
                    <tr key={q.question_id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="p-4 pl-6 font-semibold text-slate-600 whitespace-nowrap">{q.title}</td>
                      ...
                      <td className="p-4 pr-6">
                        <div className="flex items-center justify-center gap-3">
                          <button onClick={() => navigate(`/student-compiler-questions/view/${q.question_id}`)} className="text-indigo-500 hover:text-indigo-700 transition" title="View">
                            <FiEye className="w-4 h-4" />
                          </button>
                          <button onClick={() => navigate(`/student-compiler-questions/edit/${q.question_id}`)} className="text-orange-500 hover:text-orange-700 transition" title="Edit">
...
export default StudentCompilerQuestions;
