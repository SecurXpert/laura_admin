...
const ViewGuestCompilerQuestion = () => {
  ...
  useEffect(() => {
    const fetchQuestionDetails = async () => {
      try {
        const res = await api.get(`/guest/compiler-questions/get?question_id=${id}`);
        let data = null;
        ...
        if (data) {
          ...
          navigate("/guest-compiler-questions");
        }
      } catch (error) {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto overflow-x-hidden pb-12">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <button
          onClick={() => navigate("/guest-compiler-questions")}
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="p-6 md:p-8 space-y-6">
            <div>
              <p className="text-sm font-bold text-slate-600 uppercase tracking-wider mb-2">Question ID</p>
            ...
            <div>
              <p className="text-sm font-bold text-slate-600 uppercase tracking-wider mb-2">Title</p>
              <p className="text-lg font-bold text-slate-800">{questionData.title || "--"}</p>
            </div>
            
            <div>
              <p className="text-sm font-bold text-slate-600 uppercase tracking-wider mb-2">Question</p>
              <p className="text-base text-slate-700 leading-relaxed bg-gray-50 p-4 rounded-xl border border-gray-100">{questionData.question || "--"}</p>
            </div>
            
            <div>
              <p className="text-sm font-bold text-slate-600 uppercase tracking-wider mb-2">Description</p>
              <p className="text-base text-slate-700 leading-relaxed">{questionData.description || "--"}</p>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <p className="text-sm font-bold text-slate-600 uppercase tracking-wider mb-2">Sample Inputs</p>
                <div className="bg-gray-50 p-3 rounded-lg border border-gray-100 text-slate-700 font-mono text-base break-all">
                  {questionData.sample_inputs || "--"}
                </div>
              </div>
              <div>
                <p className="text-sm font-bold text-slate-600 uppercase tracking-wider mb-2">Sample Outputs</p>
                <div className="bg-gray-50 p-3 rounded-lg border border-gray-100 text-slate-700 font-mono text-base break-all">
